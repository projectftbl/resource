...
const extractData = response => response.data
    , config = { withCredentials: true, 'x-original-host': window.location.origin };
...
const getConfig = () => {
  ...
  return assign({}, config, { headers: { 'x-session-token': token, 'x-original-host': window.location.origin }});
};
