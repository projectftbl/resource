...
export default {
  post(url, options = {}) {
    const data = (options.contextType === FORM_URL_ENCODED && options.data) 
               ? qs.stringify(options.data) : options.data;

    return request.post(`/api${buildUrl(url, options)}`, data, getConfig(options)).then(extractData);
  }
...
, put(url, options = {}) {
    ...
    return request.put(`/api${buildUrl(url, options)}`, data, getConfig(options)).then(extractData);
  }
